...
from thin_tts.process_ckpt import get_sovits_version_from_path_fast, load_sovits_new
from transformers import AutoModelForMaskedLM, AutoTokenizer
from thin_tts.i18n.i18n import I18nAuto, scan_language_list
...
class TTS_Config:
    ...
    configs: dict = None
    languages: list = ["auto", "zh", "all_zh"]
    mute_tokens: dict = {"v2ProPlus": 486}
    ...
    def __init__(self, configs: Union[dict, str] = None):
        ...
        self.version = self.configs.get("version", "v2ProPlus")
        ...
        if (self.vits_weights_path in [None, ""]) or (not os.path.exists(self.vits_weights_path)):
            self.vits_weights_path = self.default_configs["v2ProPlus"]["vits_weights_path"]
            print(f"fall back to default vits_weights_path: {self.vits_weights_path}")
        if (self.bert_base_path in [None, ""]) or (not os.path.exists(self.bert_base_path)):
            self.bert_base_path = self.default_configs["v2ProPlus"]["bert_base_path"]
            print(f"fall back to default bert_base_path: {self.bert_base_path}")
        if (self.cnhuhbert_base_path in [None, ""]) or (not os.path.exists(self.cnhuhbert_base_path)):
            self.cnhuhbert_base_path = self.default_configs["v2ProPlus"]["cnhuhbert_base_path"]
            print(f"fall back to default cnhuhbert_base_path: {self.cnhuhbert_base_path}")
        if (self.sv_path in [None, ""]) or (not os.path.exists(self.sv_path)):
            self.sv_path = self.default_configs["v2ProPlus"]["sv_path"]
            print(f"fall back to default sv_path: {self.sv_path}")
        self.update_configs()
...
class TTS:
    ...
    def init_sv_model(self):
        if self.sv_model is not None:
            return
        self.sv_model = SV(self.configs.device, self.configs.is_half, self.configs.sv_path)
    ...
    def _get_ref_spec(self, ref_audio_path):
        ...
        raw_audio = torch.from_numpy(raw_audio_np).float()
        if raw_audio.dim() == 1:
    ...
    def run(self, inputs: dict):
        ...
        try:
            ...
            for item_index, item in enumerate(data):
                ...
                if self.is_v2pro:
                    cached_sv = self.prompt_cache.get("sv_emb_list")
                    if cached_sv is not None and len(cached_sv) == len(self.prompt_cache["refer_spec"]):
                        sv_emb = list(cached_sv)
                    else:
                        sv_emb = []
