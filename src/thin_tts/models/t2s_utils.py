...
def logits_to_probs(
    ...
    if previous_tokens is not None and repetition_penalty != 1.0:
        ...
        # Sliding window: only penalize recent tokens to avoid O(N) gather on full history
        rp_window = 50
        if previous_tokens.shape[1] > rp_window:
            previous_tokens = previous_tokens[:, -rp_window:]
