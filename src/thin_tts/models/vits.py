...
from thin_tts.modules import commons
from thin_tts.modules import modules
from thin_tts.modules import attentions
from torch.nn import Conv1d, ConvTranspose1d, Conv2d
from torch.nn.utils import weight_norm, remove_weight_norm, spectral_norm
from thin_tts.modules.commons import init_weights, get_padding
from thin_tts.modules.mrte_model import MRTE
from thin_tts.modules.quantize import ResidualVectorQuantizer

from thin_tts.text import symbols2 as symbols_v2
...
class SynthesizerTrn(nn.Module):
    ...
    def decode_streaming(self, codes, text, refer, noise_scale=0.5, speed=1, sv_emb=None, result_length:int=None, overlap_frames:torch.Tensor=None, padding_length:int=None, cached_ge=None):
        ...
        if cached_ge is not None:
            ge = cached_ge
        elif type(refer) == list:
            ges = []
            for idx, _refer in enumerate(refer):
                ge = get_ge(_refer, sv_emb[idx] if self.is_v2pro else None)
                ges.append(ge)
