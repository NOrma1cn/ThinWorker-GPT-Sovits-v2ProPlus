...
from thin_tts.models.t2s_utils import (
    make_pad_mask,
    make_pad_mask_left,
    sample,
    topk_sampling,
)
from thin_tts.modules.embedding import SinePositionalEmbedding, TokenEmbedding
from thin_tts.modules.transformer import LayerNorm, TransformerEncoder, TransformerEncoderLayer
...
class T2SBlock:
    ...
    def process_prompt(
        ...
        k_view = self._k_cache[:, :kv_len].view(batch_size, kv_len, self.num_heads, -1).transpose(1, 2)
        v_view = self._v_cache[:, :kv_len].view(batch_size, kv_len, self.num_heads, -1).transpose(1, 2)
        ...
        return x, self._k_cache, self._v_cache
    ...
    def decode_next_token(
        ...
        # In-place write into pre-allocated buffer instead of torch.cat
        self._k_cache[:, self._seq_len] = k.squeeze(1)
        self._v_cache[:, self._seq_len] = v.squeeze(1)
        self._seq_len += 1

        batch_size = q.shape[0]
        q_len = q.shape[1]
        kv_len = self._seq_len
        ...
        if torch_sdpa:
            attn = F.scaled_dot_product_attention(q, k_view, v_view, (~attn_mask) if attn_mask is not None else None)
        else:
            attn = scaled_dot_product_attention(q, k_view, v_view, attn_mask)
...
class Text2SemanticDecoder(nn.Module):
    ...
    def infer(
        ...
        for _ in tqdm(range(1500)):
            ...
            # 策略：文本越长，目标生成tokens越多；penalty随进度递减
            generated_len = y.shape[1] - prefix_len
            # 粗略估算：中文1字≈2-3 semantic tokens
            estimated_target = x_len * 2.5
            progress_ratio = generated_len / max(estimated_target, 1)

            # progress < 0.6: 强penalty (5.0) 防止过早停止
            # 0.6 <= progress < 1.0: 线性降低到2.0
            # progress >= 1.0: 保持2.0，允许自然结束
    ...
    def infer_panel_naive(
        ...
        **kwargs,
        ...
        # Pre-allocate buffer for generated tokens to avoid O(N²) torch.cat
        y_buf = torch.zeros(x.shape[0], MAX_SEQ_LEN, dtype=y.dtype, device=y.device)
        y_buf[:, :prefix_len] = y
        y_len_total = prefix_len
        ...
        token_counter = 0
        curr_ptr = prefix_len
        # One-time PE dtype alignment (avoid per-step implicit FP32/FP16 conversion)
        _pe_ref = y_emb if y_emb is not None else None
        if _pe_ref is not None and self.ar_audio_position.pe is not None:
