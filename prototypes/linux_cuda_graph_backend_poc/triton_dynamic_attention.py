"""Isolated Triton attention for the production single-token decode shape."""
...
def _dynamic_attention_kernel(
    ...
    # The loop bound is a device scalar. This is the property needed by an
    # outer CUDA Graph: one captured kernel can do less work for short caches.
...
def dynamic_attention(
    ...
    """Compute decode attention with seq_len read from a CUDA int32 scalar.

    q: [H, D], k_cache/v_cache: [MAX_S, H, D], output: [H, D].
    """
...
def dynamic_attention_with_kv_append(
    ...
    """Append K/V at ``seq_len - 1`` and attend through ``seq_len`` in one kernel."""
